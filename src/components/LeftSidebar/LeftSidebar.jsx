...
const LeftSidebar = () => {
    ...
    const inputHandler = async (e) => {
        // e.preventDefault;
        e.preventDefault(); // Uncomment if it's used inside a form
        ...
        try {
            ...
            if (input) {
                ...
                if (!querSnap.empty && querSnap.docs[0].data().id !== userData.id) {
                    let userExist=false;
                    chatData.map((user)=>{
                       if(user.rId === querSnap.docs[0].data().id){
                        userExist=true;
                       }
                    })
                    if(!userExist){
                        setUser(querSnap.docs[0].data());
                    }
                }
                else{
                    setUser(null);
